...
class ConversationTest < ActiveSupport::TestCase
  ...
  def test_validates_message_type_inclusion
    conversation = Conversation.new(message_type: 'invalid')
    assert_not conversation.valid?
    assert_includes conversation.errors[:message_type], "is not included in the list"
  end

  # Association tests are redundant
  ...
  def test_find_or_create_for_participants_creates_new_conversation_when_none_exists
    ...
    assert_difference 'Conversation.count', 1 do
      Conversation.find_or_create_for_participants(from, to, 'sms')
    end
  end
  ...
  def test_find_or_create_for_participants_normalizes_participant_order
    ...
    conversation = Conversation.find_or_create_for_participants(from, to, 'sms')
    assert_equal [from, to].sort, conversation.participants.pluck(:identifier).sort
  end

  def test_find_or_create_for_participants_returns_existing_conversation
    from = '+12016661234'
    to = '+18045551234'
    existing_conversation = create(:conversation, participant_identifiers: [from, to].sort, message_type: 'sms')
  ...
  def test_find_or_create_for_participants_does_not_create_new_when_exists
    from = '+12016661234'
    to = '+18045551234'
    create(:conversation, participant_identifiers: [from, to].sort, message_type: 'sms')
  ...
  def test_find_or_create_for_participants_finds_regardless_of_order
    ...
    conversation = Conversation.find_or_create_for_participants(to, from, 'sms')
    assert_equal existing_conversation, conversation
  end
  ...
  def test_other_participant_returns_correct_participant
    ...
    other_participant = conversation.other_participant('+12016661234')
    assert_equal '+18045551234', other_participant.identifier
    
    other_participant = conversation.other_participant('+18045551234')
    assert_equal '+12016661234', other_participant.identifier
  end
